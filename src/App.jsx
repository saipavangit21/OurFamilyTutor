...
import { Calculator, BookOpen, Printer, RotateCcw, CheckCircle2, XCircle, Star, ArrowLeft, ChevronLeft, ChevronRight, Sparkles, Mic, Square, Volume2 } from "lucide-react";
...
function EnglishView({ onBack }) {
  ...
  const expectedWords = useMemo(() => tokenize(passage.text).filter((t) => t.trim() !== "").map(cleanWord), [passage]);
  ...
  const processTranscript = (transcript) => {
    ...
    setWordStatus([...statusRef.current]);
    if (pointerRef.current >= expectedWords.length) stopReading();
  };
  ...
  const reviewWords = useMemo(() => {
    const toks = tokenize(passage.text).filter((t) => t.trim() !== "");
      ...
      <div style={{ background: COLORS.paper, borderRadius: 16, border: `1px solid ${COLORS.line}`, overflow: "hidden" }}>
        ...
        <div style={{ padding: "24px 28px 28px", fontFamily: "'Lexend', sans-serif" }}>
          ...
          <p style={{ fontSize: 16, lineHeight: 1.9, color: COLORS.ink }}>
            {(() => {
              let wIdx = -1;
              return tokenize(passage.text).map((tok, ti) => {
                ...
                const col = status === "correct" ? COLORS.greenDark : status === "wrong" ? COLORS.coralDark : COLORS.ink;
                return (
                  <span
                    key={ti}
                    style={{
  ...
  return (
    <div style={{ maxWidth: 760, margin: "0 auto", textAlign: "center" }}>
      ...
      <p style={{ fontFamily: "'Lexend', sans-serif", color: COLORS.inkSoft, fontSize: 15, marginBottom: 36 }}>
        Fresh worksheets and reading passages for class 2 and class 4, every time.
      </p>
